...
def test_single_volume(case, net, FLAGS,device):
    ...
    for ind in range(image.shape[0]): #iterate over channels
        slice = image[ind, :, :]
...
def Inference(FLAGS):
    with open(FLAGS.data_path + '/test.list', 'r') as f:
    ...
    for case in tqdm(image_list):
        first_metric, second_metric, third_metric = test_single_volume(case, net, FLAGS,device)
